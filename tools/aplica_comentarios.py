...
def main(argv):
    ...
    for ln in texto.splitlines():
        ...
        if m:
            ya_bloque[(int(m.group(1), 16), m.group(2).rstrip())] += 1
    ...
    for fn in sorted(glob.glob(os.path.join(carpeta, "*.notes"))):
        ...
        for ln in open(fn, encoding="utf-8").read().splitlines():
            ...
            if m.group(1) == "C":
                ...
                # Las lineas decorativas -la raya de arriba y la de abajo- se
                # repiten a proposito dentro de una misma cabecera, asi que no
                # basta con mirar si ya estan: hay que contarlas. Se lleva la
                # cuenta de cuantas hay ya en el .notes y cuantas pide la
                # tanda, y solo entran las que falten. Sin esto, cada pasada
                # del aplicador volvia a meter los marcos y las cabeceras
                # crecian solas.
                clave = (dire, cuerpo.rstrip())
                pide[clave] += 1
                if pide[clave] <= ya_bloque[clave]:
                    n_ya += 1
                    continue
            elif m.group(1) == "L":
